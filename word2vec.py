...
parser.add_argument('--iter', metavar='N', type=int, default=5, help='number of times to run')
...
# prepare corpus
sentences = LineSentence(os.path.join(data_path, 'news-summarization-train.txt'))

# run model
model = Word2Vec(sentences, size=args.size, min_count=2, window=5, sg=1, iter=args.iter, workers=4)
model.save('data/news_summarization_%d_dim.bin' % args.size)
